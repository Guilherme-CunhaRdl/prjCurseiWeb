...
async function verPost(id, desc, img, idEvento) {
    ...
    if (idEvento) {
        ...
        document.getElementById('formUpdatePost').style.width = '35%'
        ...
        document.getElementById('inicioUpdate').value = response.data[0].data_inicio_evento;
        document.getElementById('hinicioUpdate').value = response.data[0].hinicio;
        document.getElementById('fimUpdate').value = response.data[0].data_fim_evento;
        document.getElementById('hfimUpdate').value = response.data[0].data_fim_evento;
        ...
        document.getElementById('formUpdatePost').style.width = '32%'
    }
    idPost = id
...
const formUpdate = document.getElementById('formUpdatePost')
async function salvar() {
    const update = new FormData(formUpdate)
    try {
        const result = await axios.post(`http://${host}/api/cursei/postsUpdate/${idPost}`, update)
        desativarEdicao()
        carregarPost()
    } catch (error) {
