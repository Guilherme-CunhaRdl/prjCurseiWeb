        ...
        <div class="container-fluid cont">
            <div class="esquerda" style="width:100%;">
                ...
                <div class="filtros-seguidores" style="margin-bottom: 0;">
                    <input type="text" class="input-busca" placeholder="Busque pelo nome do seguidor">
                    <select class="input-ordenar">
                        ...
                        <tbody>
                            @forelse($seguidores as $seguidor)
                                <tr>
                                    <td>
                                        <div class="info-user">
                                            <img src="{{ $seguidor->foto_perfil ? asset('img/user/fotoPerfil/' . $seguidor->foto_perfil) : asset('img/user/fotoPerfil/default1.png') }}" alt="Foto" class="foto-user">
                                            <div>
                                                <div class="nome-user">{{ $seguidor->nome ?? 'Nome do usuário' }}</div>
                                                <div class="arroba-user">@ {{ $seguidor->nome_usuario ?? 'usuario' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ $seguidor->email ?? 'email@email.com' }}</td>
                                    <td>
    <!-- Botão de ver perfil -->
<button class="btn-acao" title="Ver Perfil"
    onclick="abrirModalPerfil({
        banner: '{{ asset('img/user/bannerPerfil/' . ($seguidor->banner ?? 'padrao.png')) }}',
        foto: '{{ $seguidor->foto_perfil ? asset('img/user/fotoPerfil/' . $seguidor->foto_perfil) : asset('img/user/fotoPerfil/default1.png') }}',
        nome: '{{ $seguidor->nome }}',
        arroba: '{{ $seguidor->nome_usuario }}',
       bio: '{{ $seguidor->bio ?? '' }}',
        seguidores: '{{ $seguidor->seguidores_count ?? 0 }}',
        seguindo: '{{ $seguidor->seguindo_count ?? 0 }}'
    })">
    <i class='bx bx-info-circle'></i>
</button>
                                       
<!-- Botão de remover seguidor -->
<button class="btn-acao" title="Remover Seguidor"
    onclick="abrirModalRemover('{{ $seguidor->nome_usuario }}', function() {
...
<div id="modal-remover" class="modal-seguidores" style="display:none;">
    <div class="modal-conteudo">
        <p class="modal-titulo">
            Deseja retirar <span class="modal-arroba">@<span id="modal-arroba"></span></span> de seus seguidores?
        </p>
        <div class="modal-botoes">
            <button class="btn-cancelar" onclick="fecharModal('modal-remover')">Cancelar</button>
        ...
        <div class="perfil-info-box">
            <div class="perfil-nome-arroba-box">
                ...
                <span class="perfil-bio-box" id="modal-bio">
                    Se você é jovem ainda, amanhã velho será! Se você é jovem ainda, amanhã velho será! Se você é jovem ainda, amanhã velho será!
                </span>
...
<script>
function abrirModalRemover(arroba, confirmarCallback) {
